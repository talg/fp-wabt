...
static inline void table_copy(wasm_rt_table_t* dest,
                              ...
                              u32 n) {
  if (UNLIKELY(dest_addr + (uint64_t)n > dest->size)) TRAP(OOB);
  if (UNLIKELY(src_addr + (uint64_t)n > src->size)) TRAP(OOB);
...
static void init_data_instances(Z_fac_module_instance_t *module_instance) {
}

static void init_table(Z_fac_module_instance_t* module_instance) {
}

static void init_elem_instances(Z_fac_module_instance_t *module_instance) {
}
...
void Z_fac_instantiate(Z_fac_module_instance_t* module_instance) {
  assert(wasm_rt_is_initialized());
  init_globals(module_instance);
  init_memory(module_instance);
  init_table(module_instance);
  init_data_instances(module_instance);
  init_elem_instances(module_instance);
}
